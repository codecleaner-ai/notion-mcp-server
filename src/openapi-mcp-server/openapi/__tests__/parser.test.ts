...
function verifyToolMethod(actual: ToolMethod, expected: any, toolName: string) {
  ...
  expect(actual.inputSchema, `inputSchema ${actual.name} ${toolName}`).toEqual(
    expected.inputSchema
  );
  if (expected.returnSchema) {
    expect(
      actual.returnSchema,
      `returnSchema ${actual.name} ${toolName}`
    ).toEqual(expected.returnSchema);
  }
}
...
function getParamsFromSchema(method: { inputSchema: IJsonSchema }) {
  return Object.entries(method.inputSchema.properties || {}).map(
    ([name, prop]) => {
...
describe("OpenAPIToMCPConverter", () => {
  describe("Simple API Conversion", () => {
    ...
    it("truncates tool names exceeding 64 characters", () => {
      ...
      const longNameMethod = tools.API.methods.find((m) =>
        m.name.startsWith("a".repeat(59))
      );
  ...
  describe("Complex API Conversion", () => {
    ...
    it("converts operations with query parameters", () => {
      ...
      const listPetsMethod = tools.API.methods.find(
        (m) => m.name === "listPets"
      );
    ...
    it("converts operations with request bodies using $ref", () => {
      ...
      const createPetMethod = tools.API.methods.find(
        (m) => m.name === "createPet"
      );
    ...
    it("converts operations with referenced error responses", () => {
      ...
      // Verify the description includes the "Notion | " prefix from getDescription()
      // Note: Error response information is not included in the description
  ...
  describe("Complex Schema Conversion", () => {
    ...
    it("handles deeply nested object references", () => {
      ...
      const getOrgMethod = tools.API.methods.find(
        (m) => m.name === "getOrganization"
      );
    ...
    it("handles recursive array references without requiring expansion", () => {
      ...
      const updateDeptMethod = tools.API.methods.find(
        (m) => m.name === "updateDepartment"
      );
    ...
    it("handles complex nested object hierarchies without expansion", () => {
      ...
      const getDeptMethod = tools.API.methods.find(
        (m) => m.name === "getDepartment"
      );
