...
// Skip integration tests - requires petstore-server.cjs which is not available
// TODO: Re-enable when petstore-server.cjs is added to the examples directory
...
// Skip integration tests - requires petstore-server.cjs which is not available
// TODO: Re-enable when petstore-server.cjs is added to the examples directory
describe.skip("HttpClient Integration Tests", () => {
  const PORT = 3456;
  ...
  beforeAll(async () => {
    ...
    // @ts-expect-error - createPetstoreServer is not imported (test is skipped)
  ...
  it("should filter pets by status", async () => {
    const operation = openApiSpec.paths["/pets"]
      ?.get as OpenAPIV3.OperationObject & { method: string; path: string };
    if (!operation) throw new Error("Operation not found");

    const response = await client.executeOperation<Pet[]>(operation, {
      status: "available",
    });
  ...
  it("should get a specific pet by ID", async () => {
    const operation = openApiSpec.paths["/pets/{id}"]
      ?.get as OpenAPIV3.OperationObject & { method: string; path: string };
    if (!operation) throw new Error("Operation not found");
  ...
  it("should create a new pet", async () => {
    const operation = openApiSpec.paths["/pets"]
      ?.post as OpenAPIV3.OperationObject & { method: string; path: string };
    if (!operation) throw new Error("Operation not found");
  ...
  it("should delete a pet", async () => {
    ...
    const createResponse = await client.executeOperation<Pet>(
      createOperation as OpenAPIV3.OperationObject & {
        method: string;
        path: string;
      },
      {
    ...
    const deleteResponse = await client.executeOperation(
      deleteOperation as OpenAPIV3.OperationObject & {
        method: string;
        path: string;
      },
      {
    ...
    try {
      await client.executeOperation(
        getOperation as OpenAPIV3.OperationObject & {
          method: string;
          path: string;
        },
        { id: petId }
